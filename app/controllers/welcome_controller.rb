...
require 'number_to_cn'
  ...
  def index
    ...
    @last_ten_funds   = Fund.where(state: ['gathering', 'reached', 'running', 'finished', 'closed']).order("find_in_set(state, 'closed, reached, finished, running, gathering')").order(created_at: :desc).last(10)
